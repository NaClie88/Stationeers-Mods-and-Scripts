...
| Phase Change Separator (multi-device system, not a single device) | n/a | **Two variants done** — see `phase-change-separator/`. Real Stationpedia-derived data backs 11 gases (`condensation_reference.md`). A single-chamber/Active-Vent variant (`phase_separator.ic10`) and the real two-chip Condensation/Evaporation Chamber system (`two-chamber-system/`, dial-selected target gas, AC-driven heat exchange) matching the project owner's own proven manual build. A gas-selection display was attempted and reverted — see `two-chamber-system/two_chamber_notes.md`'s "Display" section — it assumed a device that turned out to likely be Console-card-mounted, not a freestanding pin device; `d5` stays free until that's actually confirmed. Precooling (skimming easy condensate before full separation) documented as a pattern reusing the existing Air Conditioner script, no new code needed. |
